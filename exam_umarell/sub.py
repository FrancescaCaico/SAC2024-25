...
PROJECT_ID = "examuc"    # Sostituisci con il tuo Project ID
SUBSCRIPTION_ID = "newCantiereSub"
...
def callback(message):
    try:
        ...
        # Controlla se ci sono CAP da filtrare
        if not FILTER_CAP or cap in FILTER_CAP:
            print(f"[NOTIFICA] Cantiere trovato con CAP {cap}: {dati}")
        else:
            print(f"[IGNORATO] CAP {cap} non corrisponde ai filtri: {FILTER_CAP}")
